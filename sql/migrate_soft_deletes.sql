-- SQL Queries to Migrate Soft-Deleted Records to deleted_visits Table
-- This will centralize all deleted records with a dataSource field for filtering

-- First, let's modify the deleted_visits table structure to accommodate all record types
-- Add necessary columns to deleted_visits table
ALTER TABLE deleted_visits 
...
ADD COLUMN IF NOT EXISTS record_description TEXT,
ADD COLUMN IF NOT EXISTS deleted_by TEXT DEFAULT 'System',
ADD COLUMN IF NOT EXISTS deleted_reason TEXT;

-- Update existing deleted_visits records to have datasource
UPDATE deleted_visits 
SET datasource = 'visits' 
...
SELECT 
    ...
    l.npi as casenumber, -- Using NPI as a reference
...
-- This will ensure future soft deletes are automatically tracked
...
-- Useful queries for filtering deleted records by dataSource:

-- Get all deleted locations
-- SELECT * FROM deleted_visits WHERE dataSource = 'locations' ORDER BY EventDate DESC;

-- Get all deleted providers
-- SELECT * FROM deleted_visits WHERE dataSource = 'providers' ORDER BY EventDate DESC;

-- Get all deleted records by speciality
-- SELECT * FROM deleted_visits WHERE speciality = 'Cardiology' ORDER BY EventDate DESC;

-- Get count of deleted records by dataSource
-- SELECT dataSource, COUNT(*) as deleted_count 
-- FROM deleted_visits 
-- GROUP BY dataSource 
